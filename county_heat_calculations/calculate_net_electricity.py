...
def calculate_net_electricity(cbp_matching, ipf_results_formatted):
    ...
    net_electricity : pandas DataFrame
        DataFrame of estimated net electricity values by


    """
    empsize_dict = {'Under 50':'n1_49','50-99':'n50_99','100-249':'n100_249',
    ...
    net_electricity.set_index(['MECS_Region','MECS_NAICS_dummies','Emp_Size'],
                               inplace=True)

    cbp_grpd = cbp_matching.groupby(['MECS_Region','fips_matching','naics',
                                     'MECS_NAICS_dummies'],as_index=False).sum()
    ...
    cbp_grpd.set_index(['MECS_Region','MECS_NAICS_dummies','Emp_Size', 'naics',
                        'fips_matching'],inplace=True)
    ...
    net_electricity.reset_index(inplace=True)
    net_electricity.rename(columns={'energy':'MMBtu_TOTAL'}, inplace=True)

    return net_electricity

net_electricity = calculate_net_electricity(cbp_matching, ipf_results_formatted)
