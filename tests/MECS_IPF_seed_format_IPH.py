...
class Format_seed:
    ...
    def __init__(self, year=2014):
        ...
        self.url_3_2 =\
            ...
            str(self.year) + '/xls/table3_2.xlsx'

        self.url_3_3 = \
            'https://www.eia.gov/consumption/manufacturing/data/'+\
            str(self.year) + '/xls/table3_3.xlsx'

        self.seed_file = 'IPF_seed_' + str(self.year) + '.csv'

        def create_unformatted_mecs():
            ...
            writer = pd.ExcelWriter(
                    self.filepath+'MECS'+str(self.year)+'_unformatted.xlsx'
                    )
        ...
        if 'MECS'+str(self.year)+'_formatted.xlsx' not in os.listdir(
                self.filepath
                ):
        ...
        self.table3_2 = pd.read_excel(
                self.filepath + "MECS"+str(self.year)+"_formatted.xlsx",
                sheet_name='Table3.2'
                )
    ...
    def seed_correct_MECS(self, seed_df):
        ...
        table3_2_mask.reindex(seed_df.index, copy=False)
