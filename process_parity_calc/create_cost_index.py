...
# =============================================================================
    cost_index = pd.read_csv(".\calculation_data\cost_index_data.csv", index_col = 0)


    def index_mult(index_name, year1, year2 = False):
            
        try:
            if not year2:
                year2 = cost_index.loc["CE INDEX",:].last_valid_index()
            return cost_index.loc[index_name, str(year2)] / cost_index.loc[index_name, str(year1)]
            
        except KeyError:
                
            if year1 < int(cost_index.columns[0]) or year2 > int(cost_index.columns[-1]):
                    
                print("Please pick year between {} to {}".format(
                        cost_index.columns[0], cost_index.columns[-1]))
            else:
                    
                print("Not a valid cost index.")    
    index_mult("CE INDEX", 1978)
