...
To-do: adjust pdcs based on time zone work to fill in default states (not in markets)
"""
...
df = pd.read_csv("state_list.csv", index_col = "State")
pdc_dict = {}
...
for i in isone_dict.keys():
    ...
    for j in datelist:
        isone_data = pd.read_csv("./ISONE/" + str(i) + "/whlsecost_hourly_" + isone_dict[i] + "_" + j + ".csv",
...
ercot_data = pd.read_excel("./ERCOT/ERCOT.xlsx", sheet_name = None)
ercot_pdc = []
...
caiso_pdc = []
for i in range(1,14):
    caiso_data = pd.read_csv("./CAISO/CAISO_" + str(i) + ".csv")
...
for i in ["Jan-Mar", "Apr-Jun", "Jul-Sep", "Oct-Dec"]:
    miso_data = pd.read_csv("./MISO/2019_" + i + "_RT_LMPs_MISO/2019_" + i + "_RT_LMPs.csv")
...
# ADD NYISO
nyiso_data = pd.read_csv("./NYISO/NYISO.csv")
...
# ADD PJM
pjm_data = pd.read_csv("./PJM/PJM_2019_data.csv", usecols = ["datetime_beginning_utc", "pnode_name", "total_lmp_rt"])
...
spp_pdc = []
for i in range(1,13):
    spp_data = pd.read_csv("./SPP/SPP_" + str(i) + ".csv")
