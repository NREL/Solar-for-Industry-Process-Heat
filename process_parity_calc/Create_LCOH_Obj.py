...
import datetime 
import pandas as pd
#import models #contains all the capital, OM costs as a function of the yield energy. Also a factory

#purpose of this class is so that any new instantiation must define these methods
#even if they don't have access to this file
    ...
    def __init__(self,format):
        ...
        while True:
            ...
            try:           
                ...
                self.county = str(input("Enter a FIPS code or USA for national: ")).strip()
        ...
        self.state_name = fips_data.loc[fips_data['COUNTY_FIPS'] \
                               == self.county, 'State'].values[0].strip()
        
        self.state_abbr = fips_data.loc[fips_data['COUNTY_FIPS'] \
                               == self.county, 'Abbrev'].values[0].strip()
        
        #Initialize appropriate attributes based on iteration variables
        
        self.capital = 0 #get value from model.py
        self.fuel_price, self.fuel_year = gap.UpdateParams.get_fuel_price(self.county)
# =============================================================================
#         if self.iter_name == "FUELPRICE":
#             #placeholder
#             self.fixed_capital = 0 #get value from model #leave list unpacking to model
#             self.i_fuel_price = None
#             
#             #model takes temp_bin, load_bin and returns a matrix of parameter values
#             
#         if self.iter_name == "INVESTMENT":
#             #manually inputing fuel type - default ng
#             self.static_fuel_price, self.fuel_year = gap.UpdateParams.get_fuel_price(self.county)
#             self.i_capital = None
#             
# =============================================================================
        #get period of analysis
            
        while True:
            
            try:
                
                self.p_time = int(input("Enter period of analysis (integer): "))
    ...
    def import_param(self):
        ...
        def get_subsidies():
            ...
            if self.tech_type in ['BOILER','FURNACE','KILN','OVEN']:
                ...
                sol_subs = pd.read_csv(os.path.join(LCOH.path,
                    "Sol_Subs.csv"),index_col = ['State'])
        ...
        def get_corptax():
            ...
            corp_tax = pd.read_csv(os.path.join(LCOH.path,"corp_tax.csv"),
                                   index_col = ['State'], usecols = ['State', 'Total'])
        ...
        def get_dep_value(t , no_years = 5):
            ...
            if (no_years == 3):
                ...
                schedule = [0.1429,0.2449, 0.1749, 0.1249, 0.0893, 0.0892, 0.0893, 0.0446]
            elif (no_years == 10):
                schedule = [0.1, 0.18, 0.144, 0.1152, 0.0922, 0.0737, 0.0655, 0.0655, 0.0656, 0.0655, 0.0328]
            else:
                schedule = [0.2, 0.32, 0.1920, 0.1152, 0.1152, 0.0576]
        ...
        def get_OM(t):
            ...
            """ All placeholders, OM fixed/var should be obtained from model.py"""
    ...
    def calculate_LCOH(self):
        ...
        for i in range(1,self.p_time+1):
            ...
            d_cost = (self.OM(i) *(1 - self.corp_tax) - self.capital*self.depreciation(i)*self.corp_tax)/(1+self.discount_rate)**i
