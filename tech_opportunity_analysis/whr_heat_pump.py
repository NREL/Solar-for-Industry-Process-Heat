...
class WHR_HP:
    ...
    def calc_process_energy(self): 
        ...
        boiler_chp.loc[:,'whr_MMBtu'] = boiler_chp.apply(
            lambda x: x['MMBtu']*(self.wh_frac_bchp_dict.get(x['naics_sub'],0) + self.wh_frac_bchp_dict.get(x['naics'],0)),axis=1)
        
        
        boiler_chp.loc[:,'proc_MMBtu'] = boiler_chp.apply(
            lambda x: x['whr_MMBtu']*\
                (1 + 1/((self.cop_bchp_dict.get(x['naics_sub'],0)+self.cop_bchp_dict.get(x['naics'],0))-1)),axis=1)

        
        ph.loc[:,'whr_MMBtu'] = ph.apply(
            lambda x: x['MMBtu']*(self.wh_frac_ph_dict.get(x['naics_sub'],0) + self.wh_frac_ph_dict.get(x['naics'],0)),axis=1)
